...
for l in ll:
    ...
    if dir == 0:  # up
        segments.append(((x, y), (x, y-step)))
        y -= step
    elif dir == 1:  # right
        segments.append(((x, y), (x+step, y)))
        x += step
    elif dir == 2:  # down
        segments.append(((x, y), (x, y+step)))
        y += step
...
print(f"min_x: {min_x}, max_x: {max_x}, min_y: {min_y}, max_y: {max_y}")
    ...
    '''
'''
Use compressed coordinates to build wall
'''
...
for (x1,y1),(x2,y2) in segments:
    xs.update([x1,x2])
    ys.update([y1,y2])
xs.add(0); ys.add(0)   # include start
xs.add(x); ys.add(y)   # include end
xs = sorted(xs)
ys = sorted(ys)
...
for (x1,y1),(x2,y2) in segments:
    if x1 == x2:  # vertical
        cy1, cy2 = sorted([y_map[y1], y_map[y2]])
        cx = x_map[x1]
        for cy in range(cy1, cy2+1):
            wall_c.add((cx, cy))
    else:         # horizontal
        cx1, cx2 = sorted([x_map[x1], x_map[x2]])
        cy = y_map[y1]
        for cx in range(cx1, cx2+1):
            wall_c.add((cx, cy))

print(wall_c)
print_wall(wall_c, x_map[x], y_map[y])
